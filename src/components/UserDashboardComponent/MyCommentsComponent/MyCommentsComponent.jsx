...
const commentData = [
  {
    id: 1,
    comment: 'دوره آموزش جامع Js ',
    category: 'دوره آموزشی',
    time: '۱۴۰۳/۰۳/۰۲ , ۱۶:۲۷',
    seen: 'seen',
    status: 'pending'
  },
  {
    id: 2,
    comment: 'چگونه مدیر پروژه بهتری باشیم',
    category: 'اخبار و مقالات',
    time: '۱۴۰۳/۰۲/۰۸ , ۱۵:۲۸',
    seen: 'seen',
    status: 'approved'
  },
  {
...
const MyCommentsComponent = () => {
  ...
  const handleFilter = values => {
    ...
    const filtered = commentData.filter(
      item =>
        (comment
          ? item.comment.toLowerCase().includes(comment.toLowerCase())
          : true) && (time ? item.time === time : true)
    )
  ...
  return (
    <div className={styles.container}>
      <Formik initialValues={{ comment: '', time: '' }} onSubmit={handleFilter}>
        {() => (
          <Form>
